...
import { startStakingListener } from './jobs/stakingListener.js';
...
mongoose.connect(MONGODB_URI, {
  ...
  .then(() => {
    ...
    if (process.env.STAKING_LISTENER_ENABLED !== 'false') {
      startStakingListener().catch((error) => {
        console.error('[Staking Listener] Failed to start:', error);
      });
    } else {
      console.log('[Staking Listener] Disabled via STAKING_LISTENER_ENABLED=false');
    }
  })
  .catch((error) => {
