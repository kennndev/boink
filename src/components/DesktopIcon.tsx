...
export const DesktopIcon = ({ icon, label, onClick }: DesktopIconProps) => {
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-1 p-1 sm:p-2 hover:bg-primary/20 rounded transition-colors group 
                 w-20 sm:w-24 md:w-28 touch-target flex-shrink-0 min-h-[70px] sm:min-h-[80px]"
    >
      <img
        src={icon}
        alt={label}
      ...
      <span className="text-[10px] sm:text-xs md:text-sm text-white font-bold text-center 
                       drop-shadow-[0_1px_1px_rgba(0,0,0,0.8)] leading-tight px-0.5">
        {label}
      </span>
    </button>
  );
